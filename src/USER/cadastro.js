...
class BookRegistrationForm {
    ...
    init() {
        ...
        document.getElementById('newBookBtn').addEventListener('click', () => this.resetForm());
        ...
        requiredFields.forEach(field => {
            if (!field.value.trim()) {
                ...
                if (field.type === 'url' && !this.isValidURL(field.value)) {
                    this.showFieldError(field, 'Por favor, insira uma URL válida');
                    isValid = false;
                }
                if (field.type === 'number') {
                    ...
                    if (min && value < min) {
                        this.showFieldError(field, `O valor deve ser maior que ${min}`);
                        isValid = false;
                    }
                    if (max && value > max) {
                        this.showFieldError(field, `O valor deve ser menor que ${max}`);
                        isValid = false;
                    }
                }
            }
    ...
    async loadBooksFromServer() {
        try {
            return JSON.parse(localStorage.getItem('biblioteca_livros') || '[]');
        } catch (error) {
...
window.BookRegistry = {
    ...
    clearLibrary: () => {
        if (confirm('Tem certeza que deseja limpar toda a biblioteca? Esta ação não pode ser desfeita.')) {
